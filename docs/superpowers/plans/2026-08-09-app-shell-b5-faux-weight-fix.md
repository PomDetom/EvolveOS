# 普惠体 500 字重补齐 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 补齐 Alibaba PuHuiTi 的 500（Medium）真实字重，消除 medium 文字被 Chromium 伪粗合成（400 描边）导致的模糊。

**Architecture:** `fonts.css` 新增第三个 `@font-face`（`font-weight: 500`，指向 65 Medium WOFF2，与既有 55/85 同结构）；`--font-weight-medium: 500` 令牌定义不动，所有消费点自动经字重匹配命中真 65 字重。600 就近匹配 700 真粗体、不糊，不动。

**Tech Stack:** Vite + 原生 CSS + Vitest + Playwright（零运行时依赖，现有）。
...
### Task B5-F1: 补齐 500 字重（65 Medium）
