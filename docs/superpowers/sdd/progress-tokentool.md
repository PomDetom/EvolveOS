...
- **真机 bug（tauri:dev 报 `Permission core:screen:allow-current-monitor not found`）**：monitor API 在 **Window 类**（`win.currentMonitor()`），无独立 `screen` 模块，权限标识为 **`core:window:allow-current-monitor`**（不是 `core:screen:...`）。mock 显式提供了 `screen` 模块掩盖了真机失效（真实桌面下 getMonitor 恒 null → 特性静默不启用）。修复：`strip-main.js` `getMonitor` 改 `win.currentMonitor?.()`；capabilities/测试/文档权限标识改 `core:window:allow-current-monitor`；e2e mock 的 `screen.currentMonitor` 移到 getCurrentWindow 对象。分支 `ui/strip-edge-dock-fix`（自 dev），改完合并。
