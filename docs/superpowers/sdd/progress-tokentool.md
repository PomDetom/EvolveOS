...
- **真机 bug 两连（tauri:dev 构建失败 + 特性全不生效）**：① 首版用 `core:screen:allow-current-monitor` 构建报「Permission not found」（正确标识是 `core:window:...`）；② 修权限后用 `win.currentMonitor()` / `__TAURI__.screen`，**真机实测两者均 undefined**（Tauri 全局 shim 不暴露 monitor API；web mock 显式提供 mock 掩盖了真机差异 → getMonitor 恒 null → 特性静默不启用）。最终修复：**显示器 bounds 走 Rust 命令 `get_current_monitor`**（`window.current_monitor()` 取物理像素，注册 invoke_handler），JS `getMonitor` 用 `invoke('get_current_monitor')`（`?.()` 兼容无 core 的旧 mock）；移除已不用的 `core:window:allow-current-monitor` 权限与断言；e2e mock 改 `core.invoke` 返回 monitor；调试读条（`.c-strip__debug`，壳层禁右键无 DevTools）渲染 `[edge]` 诊断到 strip 本体。分支 `ui/strip-edge-debug` 承载真机调试，改完并入 dev。
