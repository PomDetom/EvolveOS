...
| 形态 | 入口 | 说明 |
|---|---|---|
| **应用壳**（主体） | 浏览器 `/`（默认，与 Tauri 一致）；`?mode=app` 显式 | 一体式标题栏 + 级联双滑动窗口（左=应用列表 / 右=应用目录或设置目录，纯图标栏）+ 黄金比例锚点（38.2%）+ 右下 FloatBall 悬浮条演示；手机（≤900px）底部横滑应用轮 + 全屏页面栈；内置首个真实应用 tokenTool，其余模块占位待填充；设计系统展示已内化为设置「组件」「动效」分区 |
| **FloatStrip 悬浮条** | 浏览器 `?mode=strip` | 独立悬浮条形态：横竖双形态 + 四边磁吸 + 无边框内容优先（与 app 互斥渲染），供独立透明窗口接入 |

## 特性

- **tokenTool**：DeepSeek 官方余额 / OpenCode Go 三窗口用量监测（复用 Tauri Rust 后端 + EvolveOS 组件，桌面优先 / 浏览器优雅降级）
- **亚克力质感**：rgba 层叠 + `backdrop-filter` 模糊 + 高光描边，深/浅双主题
